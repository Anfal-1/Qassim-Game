...
export default function QassimTourismGame() {
  ...
  if (gameState === 'landing') {
    return (
      <main className="min-h-screen bg-gradient-to-b from-[#1B1F5C] to-[#003F5C] text-white flex flex-col items-center justify-center px-6 py-16 relative overflow-hidden">
        {/* الشعار واسم الجمعية */}
        <div className="flex flex-col items-center mb-6">
          <img
            src="/logo.png" // ضع الشعار الفعلي داخل مجلد public
            alt="Qassim1 Logo"
            className="w-28 h-28 rounded-full shadow-lg ring-2 ring-white/30 mb-2"
          />
          <span className="text-sm text-gray-200">جمعية Qassim1 التقنية</span>
        </div>

        {/* عنوان اللعبة */}
        <h1 className="text-5xl md:text-6xl font-extrabold drop-shadow-md tracking-wide text-center">
          الوجهة الضبابية
        </h1>

        {/* وصف اللعبة */}
        <p className="text-xl md:text-2xl text-[#00E5E5] mt-4 mb-8 text-center">
          اكتشف معالم القصيم السياحية بلمسة ذكية
        </p>

        {/* زر الدخول إلى اللعبة */}
        <button
          onClick={() => setGameState('welcome')}
          className="flex items-center gap-2 bg-white text-[#1B1F5C] font-semibold text-lg px-8 py-3 rounded-full shadow-xl hover:scale-105 hover:shadow-2xl transition-transform duration-300"
        >
          <span>🚀</span> اكتشف اللعبة
        </button>

        {/* قسم تواصل معنا */}
        <div className="absolute bottom-8 text-center text-sm text-gray-300">
          <p className="mb-1">💡 هل لديك فكرة وتحتاج موقعًا مميزًا؟</p>
          <a
            href="mailto:info@qassim1.org"
            className="underline text-[#00A9B4] hover:text-white hover:font-bold transition"
          >
            تواصل معنا الآن
          </a>
        </div>
